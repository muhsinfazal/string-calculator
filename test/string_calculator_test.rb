...
class StringCalculatorTest < Minitest::Test
  ...
  def test_returns_sum_for_two_number_string_separated_by_comma
    sum = @calculator.add('1,2')

    assert_equal 3, sum
  end

  def test_returns_sum_for_multiple_number_string_separated_by_comma
    sum = @calculator.add('1,2,3,4,5')

    assert_equal 15, sum
  end

  def test_returns_sum_for_number_string_with_trailing_comma
    sum = @calculator.add('1,2,3,')

    assert_equal 6, sum
  end

  def test_returns_sum_for_number_string_with_leading_comma
    sum = @calculator.add(',1,2,3')

    assert_equal 6, sum
  end

  def test_returns_sum_for_number_string_separated_by_multiple_commas
    sum = @calculator.add('1,,,2,,3,4')

    assert_equal 10, sum
  end

  def test_returns_sum_for_number_string_containing_new_lines
    sum = @calculator.add("1\n2,3\n4")
  ...
  def test_returns_sum_for_number_string_with_multi_character_delimiter
    sum = @calculator.add("//[***]\n1***2***3")

    assert_equal 6, sum
  end

  def test_handles_multiple_delimiters
    sum = @calculator.add("//[*][%%]\n1*2%%3")
